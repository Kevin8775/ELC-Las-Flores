...
import { BarChart3, BookOpenText, ChevronLeft, ChevronRight, CreditCard, GraduationCap, LayoutDashboard, Megaphone, MessageSquareText, Settings2, Sliders, ThumbsUp, Users2 } from "lucide-react";
...
const links = [
  ...
  { href: "/slides", label: "Carrusel", icon: Sliders },
...
export function DashboardSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-screen flex-col border-r border-white/10 bg-[linear-gradient(180deg,#13273f_0%,#1E3A5F_100%)] text-white shadow-2xl shadow-slate-950/10 transition-all duration-300
          lg:sticky lg:top-0 lg:z-auto
          ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
          ${collapsed ? "w-20 px-3 py-4" : "w-72 px-5 py-6"}
        `}
      >
          ...
          <div className="space-y-4">
            <div className={`flex items-center gap-2 ${collapsed ? "justify-center" : "justify-between"}`}>
              ...
              <button
                ...
                className={`hidden lg:flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/80 transition hover:bg-white/10 ${collapsed ? "ml-0" : "shrink-0"}`}
